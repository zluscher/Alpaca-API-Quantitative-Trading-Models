...
while True:
    ...
    # Stock_List = ['NRGU','SOXS','SQQQ','FNGD','TQQQ','NRGU','NUGT','FNGU','FAS','LABU','SOXL','CURE','TMF','IYR','SPY','QQQ','DIA','XLK','SOXX','XLE','XOP','XLY','IYR','XLB','XLU','XLP','XLF','SH','SQQQ','GDX','EFA']
    # ,'HYG','TLT','PCY'
    ...
    #                     Value3_.insert(0,0)
        ...
        #     print()
            ...
            for i in range(len(Buy)-1):
                ...
                if Fish_[i] > 1.5:
                    if fish_psar_close == 0 or Fish_[i] < Fish_[i-1]:
                        Sell[i] = 1
                        Sell[i-1] = 1
                        Sell[i-2] = 1
    ...
    #
        ...
        # ###################################
            ...
            common_length = min(len(K_), len(RSI_), len(OBV_), len(N_), len(SMMA_), len(Aroon_), len(MACD_), len(Fish_), len(Value3_), len(FishI), len(aaplv), len(aaplo), len(aaplc), len(aapll), len(aaplh))
            ...
            feature_arrays = np.column_stack((K_array, RSI_array, OBV_array, N_array, SMMA_array, Aroon_array,
                                              MACD_array, Fish_array, Value3_array, FishI_array,
            ...
            current_day_features = np.array([K_current, RSI_current, OBV_current, N_current, SMMA_current, Aroon_current,
                                      MACD_current, Fish_current, Value3_current, FishI_current,
            ...
            print(f'Predicted Outcome for the Current Day: {predicted_outcome[0]}')
