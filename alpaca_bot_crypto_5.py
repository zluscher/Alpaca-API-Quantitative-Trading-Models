...
while True:
    ...
    aapl = pdr.get_data_yahoo(Stock_Name,
                start=datetime.date.today() + datetime.timedelta(-59),
                end=datetime.date.today(), interval="5m")
    ...
    for i in range(len(Stock_List)):
        ...
        data = yfObj.history(start='2021-12-1', end=datetime.date.today())
        indic = PSAR()
    ...
    ###################################
        try:
            ...
            def calculate_order_size(cash_to_spend, latest_price):
                ...
                units_to_buy = floor(change*cash_to_spend * precision_factor / latest_price)
            ...
            if (int(Stock_Change[len(Stock_Change)- 3]) + int(Stock_Change[len(Stock_Change)- 2]) + int(Stock_Change[len(Stock_Change)- 1])) > 0:
                change = ((int(Stock_Change[len(Stock_Change)- 3]) + int(Stock_Change[len(Stock_Change)- 2]) +int(Stock_Change[len(Stock_Change)- 1]))/110)
                bs = "buy"
                print("buy")
                print(change)
            ...
            print("Percent to Buy/Sell " + str(change))
            ...
            print("Nothing to Buy/Sell")

    minutesToSleep = 1 - datetime.datetime.now().minute % 1
