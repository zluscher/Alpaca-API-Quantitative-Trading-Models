...
while True:
    ...
    Stock_List = ['TQQQ','NRGU','GDX','FNGU','SOXL','CURE','TMF','TNA']
    # Stock_List = ['SPY']
...
##########################################################################
        ...
        #
        # common_length = min(len(K_), len(RSI_), len(OBV_), len(N_), len(SMMA_), len(Aroon_), len(MACD_), len(aaplv), len(aaplo), len(aaplc), len(aapll), len(aaplh))
        #
        # # Resize each array to common_length by excluding the beginning indices
        # from sklearn.preprocessing import MinMaxScaler
        #
        # # Convert lists to numpy arrays and select the last 'common_length' elements
        ...
        #
        # # Stack the individual feature arrays horizontally
        # feature_arrays = np.column_stack((K_array, RSI_array, OBV_array, N_array, SMMA_array, Aroon_array,
        #                                   MACD_array, aaplv_array, aaplo_array, aaplc_array, aapll_array, aaplh_array))
        #
        # # Scale the features to the range of 0-10
        # scaler = MinMaxScaler(feature_range=(0, 10))
        # feature_arrays_scaled = scaler.fit_transform(feature_arrays)
        ...
        #
        # def lorenzian_distance(x, y):
        #     # Assuming x and y are numpy arrays
        #     return np.sum(np.log(1 + (x - y)**2))
        #
        # y = np.array(Buy_array)
        #
        # # Split the data into training and testing sets
        # X_train, X_test, y_train, y_test = train_test_split(feature_arrays_scaled, y, test_size=0.2, random_state=42)
        #
        ...
        # distance_matrix_train = cdist(X_train_scaled, X_train_scaled, metric=lorenzian_distance)
        # X_train_ld = np.sum(np.log(1 + np.abs(distance_matrix_train)), axis=1)
        #
        # distance_matrix_test = cdist(X_test_scaled, X_train_scaled, metric=lorenzian_distance)
        ...
        #
        # # Calculate accuracy based on whether the predicted outcomes match the actual outcomes when y_test is 1
        # accuracy = (y_test_positive == y_pred_positive).mean()
        # print(f'Accuracy: {accuracy}')
        #
        # import matplotlib.pyplot as plt
        #
        ...
        # distance_matrix_current_day = cdist(current_day_features_scaled, X_train_scaled, metric=lorenzian_distance)
        ...
        # # FishI_array = np.array(FishI[-common_length:])
        # aaplv_array = np.array(aaplv[-common_length:])
        # aaplo_array = np.array(aaplo[-common_length:])
        # aaplc_array = np.array(aaplc[-common_length:])
        # aapll_array = np.array(aapll[-common_length:])
...
# for the Buy array
        ...
        # Assume your feature arrays K_, RSI_, OBV_, etc. are defined

        # Determine the common length among all feature arrays
        common_length = min(len(K_), len(RSI_), len(OBV_), len(N_), len(SMMA_), len(Aroon_), len(MACD_), len(aaplv), len(aaplo), len(aaplc), len(aapll), len(aaplh))
        ...
        smote = SMOTE()
        X_resampled, y_resampled = smote.fit_resample(feature_arrays_scaled, y)
        ...
        param_grid = {'n_neighbors': [3, 5, 7, 9, 11]}  # Tune the number of neighbors
        grid_search = GridSearchCV(knn, param_grid, cv=5)
        grid_search.fit(X_train, y_train)
        best_k = grid_search.best_params_['n_neighbors']

        # Create and train the KNN model using the best hyperparameters
        ...
        # Create a Voting Classifier combining KNN and Random Forest
        voting_clf = VotingClassifier(estimators=[('knn', knn_model), ('rf', rf_model)], voting='soft')
        ...
        # Print the counts of misclassifications
        y_test_1_pred_0 = np.sum((y_test == 1) & (y_pred == 0))
        y_test_0_pred_1 = np.sum((y_test == 0) & (y_pred == 1))
        print("Number of times y_test is 1 and y_pred is 0:", y_test_1_pred_0)
        print("Number of times y_test is 0 and y_pred is 1:", y_test_0_pred_1)

        # Plot actual vs. predicted outcomes
        plt.figure(figsize=(20, 12))


        colors = ['green' if yt == yp else 'red' for yt, yp in zip(y_test, y_pred)]
        plt.scatter(range(len(y_test)), y_test, color=colors, label='Actual')
        plt.scatter(range(len(y_test)), y_pred, color=colors, label='Predicted')
        plt.xlabel('Sample Index')
        plt.ylabel('Outcome')
        plt.title('Actual vs. Predicted Outcomes (Buy)')
        plt.legend()
        plt.show()
        ...
        # Create an array for the current day features
        current_day_features = np.array([K_current, RSI_current, OBV_current, N_current, SMMA_current, Aroon_current,
                                          MACD_current, aaplv_current, aaplo_current, aaplc_current, aapll_current, aaplh_current])

        # Standardize the features
        current_day_features_scaled = scaler.transform(current_day_features.reshape(1, -1))

        # Use the trained Voting Classifier to predict the outcome for the current day
        predicted_outcome_Buy = voting_clf.predict(current_day_features_scaled)

        print(f'Predicted Buy Outcome for the Current Day: {predicted_outcome_Buy[0]}')
...
##################################################################################################################################
        ...
        # Iterate through y_test and y_pred
        ...
        # Use the trained Voting Classifier to predict the outcome for the current day
        predicted_outcome_Sell = voting_clf.predict(current_day_features_scaled)
...
#################################################################################################################################
    ...
    # ###################################
        try:
            ...
            #             bs = "buy"
            #         elif bs == "buy":
            #             change = 0.95 * psar_boost
            #             bs = "buy"
            #         else:
            #             change = 0.45* psar_boost
            #             bs = "buy"
            #     if (int(Stock_Change[len(Stock_Change)- 6]) +int(Stock_Change[len(Stock_Change)- 5]) + int(Stock_Change[len(Stock_Change)- 4]) + int(Stock_Change[len(Stock_Change)- 3]) + int(Stock_Change[len(Stock_Change)- 2]) +int(Stock_Change[len(Stock_Change)- 1])) < 0:
            #         if change == 0:
            #             change = (int(Stock_Change[len(Stock_Change)- 6]) +int(Stock_Change[len(Stock_Change)- 5]) + int(Stock_Change[len(Stock_Change)- 4]) + int(Stock_Change[len(Stock_Change)- 3]) + int(Stock_Change[len(Stock_Change)- 2]) +int(Stock_Change[len(Stock_Change)- 1]))/110
