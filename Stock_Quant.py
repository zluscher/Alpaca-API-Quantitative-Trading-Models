...
while True:
    ...
    # Stock_List = ['SOXS','SQQQ','FNGD','TQQQ','NRGU','NUGT','FNGU','FAS','LABU','SOXL','CURE','TMF','IYR','SPY','QQQ','DIA','XLK','SOXX','XLE','XOP','XLY','IYR','XLB','XLU','XLP','XLF','SH','SQQQ','GDX','EFA']
    # ,'HYG','TLT','PCY'
    ...
    for i in range(len(Stock_List)):
        ...
        aapl = pdr.get_data_yahoo(Stock_Name,
                              start=datetime.date.today() + datetime.timedelta(-50),
                              end=datetime.date.today(), interval="1d")
        length = len(aapl)
    ...
    ################################################################
        ...
        common_length = min(len(K_), len(RSI_), len(OBV_), len(N_), len(SMMA_), len(Aroon_), len(MACD_), len(Fish_), len(Value3_), len(FishI_), len(aaplv), len(aaplo), len(aaplc), len(aapll), len(aaplh))
        ...
        FishI_array = np.array(FishI_[-common_length:])
